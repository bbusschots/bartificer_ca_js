...
((bartificer, $, undefined)=>{
    ...
    */
    ...
    //
    // --- The bartificer.ca.State Prototype ---
    //
    ...
    bartificer.ca.State = class{
        ...
        constructor(value, label){
            // store the value and label
            this.value(value); // could throw error
            this.label(label); // could throw error
        }
        
        /**
         * An accessor for the state's value.
         *
         * @param {PrimitiveValue} [value] - if present, a new value for this
         * state.
         * @returns {PrimitiveValue} Always returns the state's value.
         * @throws {TypeError} A type error is thrown if an invalid argument
         * is passed.
         */
        value(value){
            // set if needed
            if(arguments.length >= 1){
                if(!isPrimitiveValue(value)) throw new TypeError('if present, the new value for a state must be a primitive (boolean, number, or string)');
                this._value = value;
            }
        ...
        /**
         * An accessor for the state's label.
         *
         * @param {NonEmptyString} [label] - if present, a new label for this
         * state.
         * @returns {NonEmptystring} Always returns the state's label.
         * @throws {TypeError} A type error is thrown if an invalid argument
         * is passed.
         */
        label(label){
            // set if needed
            if(arguments.length >= 1){
                if(!isNonEmptyString(label)) throw new TypeError('if present, the new label for a state must be a string, and must be at least one character long');
                this._label = label;
            }
            
            // always return the current label
            return this._label;
        }
     ...
     */
    bartificer.ca.Cell = class{
        /**
         * The constructor expects to be passed a table data cell, and that
         * cell will be emptied of all content. The class `bartificer-ca-cell`
         * will beadded to the data cell, and a reference to the constructed
         * object will be added to the data cell as the data attribute
         * `data-bartificer-object` (`bartificerObject` from JavaScipt's point
         * of view).
         *
         * @param {jQuerySingleTD} $td - A jQuery object representing the table
         * data cell that will represent the new cell within the automaton.
         * @param {GridCoordinate} x - the cell's x coordinate within the
         * automaton.
         * @param {GridCoordinate} y - the cell's y coordinate within the
         * automaton.
         * @param {bartificer.ca.State} [s] - the cell's initial state.
        ...
        constructor($td, x, y, s){
             ...
             */
            this._state = s ? s.clone() : undefined;
        ...
        /**
         * A read-only accessor function to access the cell's x and y
         * coordinates within the automaton.
         *
        ...
        /**
         * A read-only accessor function to access the cell's current state, if
         * any.
         *
         * @returns {(bartificer.ca.State|undefined)} When the cell has a
         * current state that's returned, otherwise `undefined` is returned.
         * @throws {Error} An error is thrown if the accessor is called with
         * arguments.
         */
        state(){
            ...
            return this._state ? this._state.clone() : undefined;
        }
        
        /**
         * Accessor function for the cell's next state. Always returns the next
         * state of the cell. If an argument is passed, the next state is
         * updated to that value and then returned.
         *
         * @param {(bartificer.ca.State|undefined)} [ns] - if passed, the next
         * state of the cell is set to this value. `undefined` indicates that
         * the cell has no next state.
         * @returns {(bartificer.ca.State|undefined)} When the cell has a next
         * state that's returned, otherwise `undefined` is returned.
         * @throws {TypeError} If an argument is passed and it's not value, an
         * error is thrown.
         */
        nextState(ns){
            ...
            if(arguments.length >= 1){
                if(!(typeof ns === 'undefined' || isCellState(ns))){
                    throw new TypeError('if present, the first argument must be a cell state, or the value undefined');
                }
                this._nextState = ns ? ns.clone() : undefined;
            }
        
            // always return the current next state
            return this._nextState ? this._nextState.clone() : undefined;
        }
     ...
     *
     * The automaton uses a lower-right quadrant zero-indexed coordinate
     * system. The horizontal axis is labeled the *x axis* and numbered from
     * left to right in whole number increments. The vertical axis is labeled
     * the *y axis*, and numbered from top to bottom in whole-number
     * increments. When describing a cell it's x coordinate is specified first,
     * and its y coordinate second. Hence, the cell in the top-left corner has
     * the coordinates (0, 0), the second cell in the top row has the
     * coordiantes (1, 0), and the cell in the second cell on the left-most
     * column has the coordinates (0, 1).
     *
    ...
    bartificer.ca.Automaton = class{
         ...
         * takes.
         * @param {renderFunction} renderFn - a callback that will be used to
         * render the state of each cell.
         * @param {(bartificer.ca.State|bartificer.ca.State[]|initialisationFunction)} [s] - the
        ...
        /**
         * A read-only accessor function to access a reference to the jQuery
         * object representing the container that holds the table that
         * represents the automaton within the DOM.
         *
        ...
        /**
         * A read-only accessor function to access a reference to the jQuery
         * object representing the table that represents the automaton within
         * the DOM.
         *
        ...
        /**
         * A read-only accessor function to get the number of columns within
         * the automaton.
         *
         * @returns {GridDimension}
         * @throws {Error} An error is thrown if the accessor is called with
        ...
        /**
         * A read-only accessor function to return both the width and height of
         * the grid, i.e. the number of rows and columns.
         *
         * @returns {number[]} An array of two numbers, the width (number of
         * cols), and height (number of rows).
         * @throws {Error} An error is thrown if the accessor is called with
         * arguments.
         */
        dimensions(){
        ...
        /**
         * A function for adding a callback to be executed when ever the
         * generation changes, or, to execute all registered geneation-change
         * callbacks.
         *
         * When called with no parameters all callbacks are execute, when
         * called with a callback as the first parameter, that callback is
         * registered.
         *
         * @param {function} [fn]
         * @returns {bartificer.ca.CellularAutomaton} Returns a reference to
         * self.
         * @throws {TypeError} An error is thrown if called with parameters,
         * and the first parameter is not a callback.
         */
        generationChange(fn){
         ...
         * @returns {bartificer.ca.Cell} a reference to the object representing
         * the requested cell.
         * @throws {TypeError} A type error is thrown if invalid arguments are
         * passed.
         * @throws {RangeError} A range error is thrown if the coordinates
         * specify a cell beyond the edges of the automaton.
         */
        cell(x, y){
        ...
        /**
         * A function for accessing the current state of a given cell within
         * the automaton.
         *
         * @param {GridCoordinate} x - the x coordinate of the requested cell.
         * @param {GridCoordinate} y - the y coordinate of the requested cell.
         ...
         * @throws {TypeError} A type error is thrown if invalid arguments are
         * passed.
         * @throws {RangeError} A range error is thrown if either of the
         * coordinates specify a cell who's coordiantes are beyond the edges of
         * the automaton.
         */
        cellState(x, y){
         ...
         *
         * @param {(bartificer.cs.State|bartificer.ca.State[]|initialisationFunction)} [newState] -
         * a new state for the automaton. The state can be specified in three
         * different ways:
         * 1. a single state - each cell in the automaton will be initialised
         *    with this state.
         * 2. an array of states. The array must have the same dimensions as
         ...
         * @param {GridCoordinate} x - the x coordinate of the requested cell.
         * @param {GridCoordinate} y - the y coordinate of the requested cell.
         * @returns {CellState[]} Returns an array of eight cell states where
         * the state at index zero is that from the cell directly above the
         * current cell, and then moving clockwise from there. If a cell is at
         * an edge, the indexes for the non-existent positions will still be
         * included in the array, but they will have the value `null`.
         * @throws {TypeError} A type error is thrown if invalid arguments are
         * passed.
         * @throws {RangeError} A range error is thrown if the coordinates specify
         * a cell beyond the edges of the automaton.
         */
        cellNeighbourStates(x, y){
         ...
         * milliseconds.
         * @returns {bartificer.ca.CellylarAutomaton} Returns a reference to
         * self.
         * @throws {TypeError} A type error is thrown if the optoinal parameter
         * is present, but not a whole number greater than zero.
         */
        start(ms){
